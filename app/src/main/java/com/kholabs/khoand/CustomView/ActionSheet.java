package com.kholabs.khoand.CustomView;
...
public class ActionSheet extends Fragment implements OnClickListener {
	...
	private static final String ARG_VISIBILITY_IMAGEVIEW = "visible_imageview";
	private static final String ARG_IMAGE_BITMAP = "thumbnail_image";
	...
	private final int IMAGE_VIEW_ID = 400;
	...
	private boolean isImageVisiblity = false;
	...
	private void createItems() {
		...
		isImageVisiblity = getVisibleImageView();

		if (isImageVisiblity)
		{
			ImageView ivPath = new ImageView(getActivity());
			ivPath.setId(IMAGE_VIEW_ID);

			if (bmp != null) {
				ivPath.setImageBitmap(bmp);
			}
			ivPath.setScaleType(ImageView.ScaleType.CENTER);
			LinearLayout.LayoutParams params = createImageLayoutParams();
			params.topMargin = mAttrs.otherButtonSpacing;
			params.bottomMargin = mAttrs.otherButtonSpacing;
			mPanel.addView(ivPath, params);
	...
	public LinearLayout.LayoutParams createImageLayoutParams() {
		LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(
				mAttrs.imageViewSize, mAttrs.imageViewSize);
	...
	private Attributes readAttribute() {
		...
		attrs.imageViewSize = (int) a.getDimension(R.styleable.ActionSheet_actionSheetImageSize, attrs.imageViewSize);
	...
	private boolean getVisibleImageView() {
		return getArguments().getBoolean(ARG_VISIBILITY_IMAGEVIEW);
	}

	private Bitmap getImageBitmap() {
		Bitmap bmp = getArguments().getParcelable(ARG_IMAGE_BITMAP);
		return bmp;
	}
	...
	public static class Builder {
		...
		public Builder setImageVisiblity(boolean _visibility) {
			mImageVisibility = _visibility;
			return this;
		}
		...
		public Bundle prepareArguments() {
			...
			bundle.putParcelable(ARG_IMAGE_BITMAP, mImageBitmap);
			bundle.putBoolean(ARG_VISIBILITY_IMAGEVIEW, mImageVisibility);
			return bundle;
		}
